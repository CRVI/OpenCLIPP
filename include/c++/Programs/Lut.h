...
public:
   ...
   void LUT(IImage& Source, IImage& Dest, uint * levels, uint * values, uint NbValues);
   ...
   /// \param levels : Contains NbValues describing the levels to look at in Source
   /// \param values : Contains NbValues describing the values to use for those levels
   void LUTLinear(IImage& Source, IImage& Dest, float * levels, float * values, uint NbValues);

   /// Scales values of Source image according to the given input and output ranges
   void Scale(IImage& Source, IImage& Dest, float SrcMin, float SrcMax, float DstMin, float DstMax);
};
